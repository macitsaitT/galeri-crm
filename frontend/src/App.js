    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden border border-neutral-100 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(e)=>{e.preventDefault(); setProfile(formData); onClose();}} className="p-6 space-y-4">
          ...
          <div><label className="block text-xs font-bold text-neutral-500 uppercase mb-1">Telefon</label><div className="relative"><span className="absolute left-3 top-2.5 text-neutral-400"><Phone size={18}/></span><input type="text" className="w-full pl-10 pr-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-yellow-500 outline-none" placeholder="0(5XX)XXXXXXX" value={formData.phone} onChange={e=>setFormData({...formData, phone: handlePhoneInput(e.target.value)})}/></div></div>
...
const AddCustomerModal = ({ isOpen, onClose, newCustomer, setNewCustomer, onSave, inventory }) => {
  if (!isOpen) return null;
